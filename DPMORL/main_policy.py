...
def make_env(env_name, rank, utility_function, reward_dim, reward_dim_indices, seed=None):
    ...
    def _init():
        ...
        # Speed-focused config for highway
        if "highway" in str(env):
            ...
            cfg.update({
                ...
                "lanes_count": 4,
                "duration": 20, #30              # episode length in seconds (sim steps = duration*policy_frequency)
                # cheap observations
        ...
        env = ObsInfoWrapper(env, reward_dim=reward_dim,
...
if __name__ == "__main__":
    ...
    print(f"env: {args.env}, env reward dim: {reward_dim}")
    ...
    if (args.num_policies == 0):
        ...
        num_total_policies = args.num_policies
    ...
    if not args.test_only:
        for policy_idx in range(num_total_policies):
            ...
            """env_fns = [
                make_env(gym_id_name, i, None, reward_dim=reward_shape,
                        reward_dim_indices=reward_dim_indices, seed=args.seed + i)
            ...
            # keep your wrapper above the VecEnv (no pickling issues)
            env = MultiEnv_UtilityFunction(env, utility_function,
                                        reward_dim=reward_shape,
