...
def plot_scatter(env_name, prefix, save_dir, 
    ...
    if plot_final:
        ...
        for i, file_path in enumerate(paths): 
            ...
            handles.append(sc)
            labels.append(f"Policy {i} ({name})")

        plt.title('Return Distribution of Final Policies')

        # Add spaced axis labels
        plt.xlabel('Return 1', labelpad=20)
        plt.ylabel('Return 2', labelpad=20)
        ...
        plt.savefig(f'{save_sub_dir}/test_final_batch_{batch_size}.png',
        ...
        for i, policy_index in enumerate(selected_policies):
            ...
            episode_vec_returns = np.load(file_path)['episode_vec_returns']
            final_episode_vec_returns = episode_vec_returns[-100 * batch_size:, :2]
        ...
        plt.savefig(os.path.join(save_sub_dir, f'{env_name}_final_batch_{batch_size}.png'),
                    dpi=160, bbox_inches='tight')
        plt.clf()        
