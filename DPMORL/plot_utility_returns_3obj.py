...
assert len(sys.argv) >= 2, 'Please run "python plot_utility_returns_3obj.py [exp_name]". '
...
def plot_scatter(env_name, prefix, save_dir,
    ...
    if len(min_vals) > 0:
        min_vals = np.min(min_vals, 0)[:3]
        max_vals = np.max(max_vals, 0)[:3]
    ...
    if plot_learning_curve:
        # optional learning-curve scatter across training
        if plot_history_data:
            for file_path in glob.glob(f'{save_sub_dir}/MORL*.npz'):
                ...
                if os.path.exists(file_path):
                    print(file_path)
                    episode_vec_returns = np.load(file_path)['episode_vec_returns']  # [T, 3]
                    ...
                    # batch over time
                    episode_batches = [
                        np.mean(episode_vec_returns[i:i + batch_size], 0)
                        for i in range(0, len(episode_vec_returns), batch_size)
                    ]  # [B, 3]
                    ...
                    if env_name in normalization_data:
                        x_min, y_min, z_min = normalization_data[env_name]['min'][0, :3]
                        x_max, y_max, z_max = normalization_data[env_name]['max'][0, :3]
                        ax.set_xlim(x_min, x_max)
                        ax.set_ylim(y_min, y_max)
                        ax.set_zlim(z_min, z_max)

                    ax.set_title(f'{env_name} - {algo_name} - {utility_name} - 3D Return Scatter')

                    sm = plt.cm.ScalarMappable(
                        cmap=cm.rainbow,
                        norm=mcolors.Normalize(vmin=0, vmax=len(episode_batches) - 1)
                    )
                    cbar = plt.colorbar(sm, ax=ax)
                    cbar.ax.set_ylabel('Episode', rotation=270, labelpad=15)
                    plt.savefig(os.path.join(save_sub_dir, f'{name}_b{batch_size}_3d.png'),
                                bbox_inches='tight')
                    plt.close()

        # per-policy test scatter
        for file_path in glob.glob(f'{save_sub_dir}/test_returns*.npz'):
            ...
            episode_vec_returns = np.load(file_path)['test_returns']  # [N, 3]

            fig = plt.figure(figsize=(10, 6))
            ax = fig.add_subplot(111, projection='3d')

            ax.scatter(
                episode_vec_returns[:, 0],
                episode_vec_returns[:, 1],
                episode_vec_returns[:, 2],
                alpha=0.6,
                label=name,
                s=80
            )
            ...
            ax.set_ylabel('Return 2')
            ax.set_zlabel('Return 3')
            ax.set_title(rf'Return Distribution of $\pi_{number}$')
            ...
            plt.legend()
            plt.savefig(f'{save_sub_dir}/test_returns_{name}_3d.png',
                        dpi=160, bbox_inches='tight')
            plt.close(fig)

    if plot_final:
        # all test policies in one 3D scatter (batched)
        plt.clf()
        ...
        for i, file_path in enumerate(paths):
            ...
            episode_batches = [
                np.mean(episode_vec_returns[idx:idx + batch_size], 0)
                for idx in range(0, len(episode_vec_returns), batch_size)
            ]
            episode_batches = np.asarray(episode_batches)
            ...
            ax.scatter(
                ...
                alpha=0.6,
                label=f'Policy #{i}',
                marker=marker[(i // 10) % len(marker)],
                color=colors[i]
            )

        ax.set_title('Return Distribution of Final Policies')
        ax.set_xlabel('Return 1')
        ax.set_ylabel('Return 2')
        ax.set_zlabel('Return 3')
        ax.legend(loc="upper left", bbox_to_anchor=(1.1, 1), borderaxespad=0., fontsize=10)
        plt.savefig(f'{save_sub_dir}/test_final_batch_{batch_size}_3d.png',
                    dpi=160, bbox_inches='tight')
        plt.close(fig)

        # select (approx) non-dominated policies using last 500 episodes
        plt.clf()
        ...
        for file_path in file_paths:
            episode_vec_returns = np.load(file_path)['episode_vec_returns']  # [T, >=3]
            tail = episode_vec_returns[-500:, :3]
            max_returns.append(tail.max(0))
            min_returns.append(tail.min(0))
            sample_returns.append(tail)
        ...
        for i in range(len(max_returns)):
            def not_dominate_sample(current, other):
                # same logic as original, now in 3D
        ...
        # collect handles + labels for side legend
        handles = []
        labels = []
        ...
        for i, policy_index in enumerate(selected_policies):
            ...
            episode_vec_returns = np.load(file_path)['episode_vec_returns']
            final_episode_vec_returns = episode_vec_returns[-100 * batch_size:, :3]
            episode_batches = [
                np.mean(final_episode_vec_returns[idx:idx + batch_size], 0)
                for idx in range(0, len(final_episode_vec_returns), batch_size)
            ]
            episode_batches = np.asarray(episode_batches)
            ...
            sc = ax.scatter(
                ...
                episode_batches[:, 2],
                alpha=0.6,
                marker=marker[(count // 10) % len(marker)],
                color=color
            )
            handles.append(sc)
            # you can make this f'{algo_name}-{utility_name}' if you prefer
            labels.append(f'policy-{i}')
            count += 1

        ax.set_title(f'{env_name}, batch={batch_size} (3-objective)')
        ax.set_xlabel('Return 1', labelpad=20)
        ax.set_ylabel('Return 2', labelpad=20)
        ax.set_zlabel('Return 3', labelpad=25)
        ...
        plt.savefig(os.path.join(save_sub_dir, f'{env_name}_final_batch_{batch_size}_3d.png'),
                    dpi=160, bbox_inches='tight')
        plt.close(fig)
...
if __name__ == '__main__':
    ...
    for env in env_names:
        for batch_size in batch:
            ...
            plot_scatter(env, prefix, global_save_dir,
