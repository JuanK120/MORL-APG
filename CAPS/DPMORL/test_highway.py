...
def make_eval_env(gym_id_name, utility_function, reward_shape, reward_dim_indices, seed=0, augment_state=False):
    ...
    print(f"Environment created: {gym_id_name}")
    return vec, None
...
def get_utility_function(reward_shape, idx=0,linear_utility=True, lamda=0.1, keep_scale= True, max_num_policies=6):
    ...
    for path_ in pretrained_utility_paths:
...
def test(model_path, num_episodes=10, mode='ppo', augment_state=False, deterministic=True):
    ...
    return highlights_data, model, num_feats, act_dim , None
...
if __name__ == '__main__':
    # Keep the same CLI as original; model_path is unused but preserved for compatibility
