...
class Utility_Function_Programmed(nn.Module):
    def __init__(self, reward_shape=2, norm=True, lamda=0.1, function_choice=1, keep_scale=True):
        ...
        print('reward_shape in Utility_Function_Programmed',self.reward_shape)
...
class Utility_Function_Linear(nn.Module):
    def __init__(self, reward_shape=2, norm=True, lamda=0.1, function_choice=1, keep_scale=True):
        ...
        self.utility_functions = []
        print('reward_shape in Utility_Function_Linear', self.reward_shape)

        weights_list = []

        if self.reward_shape == 2: 
            weights_list = [
                [0.9, 0.1], [0.8, 0.2], [0.7, 0.3], [0.5, 0.5],
                [0.3, 0.7], [0.2, 0.8], [0.1, 0.9], [0.98, 0.02],
                [0.02, 0.98], [0.95, 0.05], [0.05, 0.95], [1.0, 0.0], [0.0, 1.0],
            ]
        else:
            # uniform preference
            weights_list.append(np.ones(self.reward_shape) / self.reward_shape)

            # per-dimension emphasis with several alphas
            alphas = [0.9, 0.7, 0.5, 0.3, 0.1]
            for i in range(self.reward_shape):
                for a in alphas:
                    w = np.full(self.reward_shape, (1.0 - a) / (self.reward_shape - 1))
                    w[i] = a
        ...
        for w in weights_list:
            ...
            self.utility_functions.append(
                lambda x, w=w: np.sum(x * np.expand_dims(w, 0), axis=1)
            )
