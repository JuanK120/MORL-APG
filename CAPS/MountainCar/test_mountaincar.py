...
def test(model_path, num_episodes=10, mode='PPO'):
    ...
    register_env('MountainCar', env_creator)
    ...
    if mode == 'PPO':
        agent = PPOTrainer(config=config, env='MountainCar')
    else:
        agent = DQNTrainer(config=config, env='MountainCar')
...
def calculate_fidelity(model_path, all_clusters, data, num_episodes=10, topin=False, apg_act=None, mode='PPO'):
    ...
    def get_cluster_action(clusters, num_feats=2, num_actions=3):
        if clusters == []:
